...
import deviceHelper from "../db/deviceHelper";
...
export async function getNotification(req: Request, res: Response) {
  try {
    ...
    const notification = await notificationHelper.get(
    ...
    return global.sendResponse(
      ...
      { data: notification[0].data, hasNextPage, hasPreviousPage }
    );
  } catch (error) {
    console.log(error);
    return global.sendResponse(
...
export async function getNotificationCount(req: Request, res: Response) {
  try {
    const notification = await notificationHelper.getCount(
      new Types.ObjectId(req.user._id)
    );
    return global.sendResponse(
      res,
      200,
      true,
      "Notification count get successfully.",
      notification
    );
  } catch (error) {
    console.log(error);
...
export async function addFCMToken(req: Request, res: Response) {
  try {
    req.body.user = req.user._id;
    await deviceHelper.insertOne(req.body);
    return global.sendResponse(res, 201, true, "fcm added successfully.");
  } catch (error) {
    console.log(error);
    return global.sendResponse(
      res,
