...
const userHelper = {
  ...
  // FindOne
  findOne: async (
    query?: { email?: string; _id?: Types.ObjectId; username?: string },
    select?: string
  ) => {
    try {
  ...
  updateMany: async (
    query: { name?: string; dob?: Date; email?: string },
    data: object,
    option: object
  ) => {
    try {
  ...
  updateOne: async (
    query: { _id?: string },
    data: { name?: string; dob?: Date; email?: string; password?: string }
  ) => {
    try {
